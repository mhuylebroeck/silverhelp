  ...
  def index #the index page for the helper
    ...
    unless query.nil?
      if params[:help_request][:location].present? && params[:help_request][:start_time].present? && params[:help_request][:duration].present? && params[:help_request][:task_id]
        ...
        redirect_to(new_help_request_path(:help_request => {:task_id =>["",query["task_id"].to_i]}))
        flash[:alert] = "Fullfill all the information in the form to be able to find helpers."
      end
    end
  end
